<template>
  <PostLayout :post="post">
    ...
    <p>
      Brave also upgrades your connections to secure HTTPS links wherever possible and includes
      simple tools like "Shields," which let you see and manage what's being blocked on a page. For
      teams communicating across multiple locations or devices, this means fewer risks of data leaks
      or phishing scams while maintaining complete control over what's loaded in your browser. In
    ...
    <p>
      Vivaldi also includes a built-in mail client (Vivaldi Mail) that lets you manage your email
      accounts directly inside your browser. You can add Gmail, Outlook, or any IMAP/POP3 email,
      organize messages with powerful search and filtering tools, and handle event invitations
      seamlessly with integrated calendar features. It supports offline search, so even with spotty
      internet, you don't lose access to important communications. For nonprofits working with large
      volumes of mail, being able to read and reply without switching apps saves time and keeps
      everyone in flow.
    </p>

    <p>
      Beyond these, Vivaldi packs smart productivity tools: a built-in notes panel for capturing web
      research, Web Panels for pinning frequently used sites, a Reader Mode for distraction-free
      reading, and automatic ad/tracker blocking. Custom keyboard shortcuts, mouse gestures, and
      even a theme editor empower everyone to work in their own style. For mission-driven teams
    ...
    <p>
      ...
      through multiple links. This is powered by a vibrant open-source community that builds these
      instant answers from credible sources such as Wikipedia. Additionally, DuckDuckGo supports
      "Bangs," shortcuts that let you search directly on popular websites like YouTube, Amazon, or
      Twitter by typing an exclamation mark followed by the site code (e.g., <code>!yt</code> for
    ...
    <p>
      ...
      how aggressively trackers are blocked. Their commitment to privacy means no IP storage or
      search history tracking, while still delivering relevant and timely results. This makes
      DuckDuckGo an excellent choice for nonprofits wanting a straightforward, privacy-first search
      experience that puts security and neutrality above personalization.
    </p>
  </PostLayout>
</template>
...
const post = {
  ...
  excerpt:
    'For nonprofits, the Web browser is a crucial tool that should provide something that almost every organization working online urgently needs today — protection from digital tracking and faster, cleaner browsing without distractions.',
}
